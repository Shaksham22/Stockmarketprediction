...
def nifty50():
    ...
    for i,a in enumerate(stockname):
        try:
            l[i]={}
            l[i]['fullname']=get_company_name(a+".NS")
            l[i]['tradingname']=a+".NS"
            today = datetime.today().strftime('%Y-%m-%d')
            one_year_ago = (datetime.today() - timedelta(days=365)).strftime('%Y-%m-%d')
...
# def get_data():
#     # Extract the closing values from Nifty50 data
#     nifty_close = nifty_data['Close']

#     # Create a list of dictionaries, each containing date and its corresponding closing value
#     data_list = [{'date': str(date.date()), 'value': float(value)} for date, value in nifty_close.items()]
